...
public class Collision {
    ...
    public Collision(PhysicsObject objA, PhysicsObject objB) {
        if (objA.equals(objB))
            throw new IllegalArgumentException("Expected objA and objB to be different");
        this.objA = objA;
        this.objB = objB;
    }
    ...
    private void PolyVsPoly(PolygonCollider a, PolygonCollider b) {
        ...
        if (penetrationA.distance > penetrationB.distance) {
            refPoly = a;
            incPoly = b;
        ...
        // Determine direction of normal. Perpendicular to the reference face
        normal = flip ? refFaceNormal : refFaceNormal.invert();

        // Does it collide at incident face vertex a
        ...
        // Does it collide at incident face vertex b
     ...
     * errors and other physics errors
     */
    private void positionalCorrection() {
        ...
        if (objA.mass != 0)
            objA.transform.translation = objA.transform.translation.subtract(correction);
